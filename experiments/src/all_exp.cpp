//
// Created by serafeim on 2/11/22.
//
...
void throughput(commandLine& command_line)
{
	...
	size_t repeat_times     = command_line.getOptionIntValue("-repeat", 3);
...
//				cout << "batch-" << b << " and batch_seed-" << batch_seed[b] << endl;
				...
				// Delete the batch of edges without changing the walks
				malin.delete_edges_batch(edges.second, edges.first, b+1, false, true, graph_size_pow2, false);
				// Deleted edges without affecting the walks
			...
			cout << "(2) average latency: " << fixed << setprecision(8) << average_latency / (n_batches * 1.0) << endl << endl;
	}
