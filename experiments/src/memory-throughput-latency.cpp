...
//			  auto pair = pairings::Szudzik<types::Vertex>::unpair(enc_triplet);
			  ...
			  auto walk_id  = enc_triplet / config::walk_length;                  // todo: needs floor?
			  auto position = enc_triplet - (walk_id * config::walk_length); // todo: position here starts from 0. verify this one!
//			  auto next_vertex   = pair.second;
//				cout << enc_triplet << " ";
			  cout << "{" << walk_id << ", " << position << "}" << " " << endl;
			});
			cout << endl;
